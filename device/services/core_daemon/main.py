...
def load_config() -> dict[str, Any]:
    config_path = os.environ.get(
        "CORE_DAEMON_CONFIG", "device/services/core_daemon/config/default.yaml"
    )
    try:
...
async def run() -> None:
    ...
    service = CoreDaemonService(cfg, bus=bus)
    await service.start()
    ...
    if use_uds:
        ...
        uds_path = socket_dir / "core-daemon.sock"
        if uds_path.exists():
            uds_path.unlink()
        config = uvicorn.Config(app=app, uds=str(uds_path), log_level="info")
    else:
        config = uvicorn.Config(
            app=app, host="0.0.0.0", port=int(cfg.get("port", 8000)), log_level="info"
        )
